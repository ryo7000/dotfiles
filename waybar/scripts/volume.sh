...
check-muted() {
  local muted
  muted=$(LC_ALL=C pactl "get-$dev_mute" "$dev" | awk '{print $2}')
...
get-volume() {
  local vol
  vol=$(LC_ALL=C pactl "get-$dev_vol" "$dev" | awk '{print $5}' | tr -d '%')
...
toggle-mute() {
  ...
  dunstify "$title: $state" -i "$icon" -r $NID
}
...
set-volume() {
  ...
  dunstify "$title: ${new_vol}%" -h int:value:$new_vol -i "$icon" -r $NID
}
